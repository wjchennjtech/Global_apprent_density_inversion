function grav_gradient = compute_gravity_gradient(final_density, up_boundary, low_boundary, height, nmax, nlat, nlon, R, G, Lat, Lon)
    ...
    for n = 0:nmax
        factor = (((R+datat2)/(R+height))^(n+3) - ((R+datat1)/(R+height))^(n+3) ) * 4*pi*G*(n+1)*(n+2)/((2*n+1)*(n+3)) ;          
    ...
    for k = 1:5  % binomial expansion
       ...
       for n = 0:nmax
            ...
            W1 = 4*pi*G*(n+1)*(n+2)*(((R+datat1)/(R+height))^(n+3))/((2*n+1)*(n+3));
            CCNM1(n + 1, :) = W1 * cnm1(n + 1, :) * C;
            SSNM1(n + 1, :) = W1 * snm1(n + 1, :) * C;
            W2 = 4*pi*G*(n+1)*(n+2)*(((R+datat2)/(R+height))^(n+3))/((2*n+1)*(n+3));
    ...
    gradient = synthesis( forwardCnm, forwardSnm, nmax, Lat, Lon);%perform spherical harmonic synthesis using the spherical harmonic coefficients
    gradient = gradient * 1e9;
    grav_gradient = zeros(nlat*nlon, 3);
    for i = 1:nlat
        for j = 1:nlon
            grav_gradient((i-1)*nlon+j,1) = Lon(j);
            grav_gradient((i-1)*nlon+j,2) = Lat(i);
            grav_gradient((i-1)*nlon+j,3) = gradient(i,j);%output the final gravity attraction data
        end
    end
end
