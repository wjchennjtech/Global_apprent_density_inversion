function [accumulated_density,gravity_rms_values] = accumulation_density_by_attraction(up_boundary, low_boundary, height, gravity_attraction, initial_density, max_iter, tol, alpha,Lon,Lat,nmax)
    ...
    for iter = 1:max_iter 
       ...
       waitbar(iter / max_iter, hWait, sprintf('%.1f%% - Iteration %d of %d', progress, iter, max_iter));%set up a progress bar
        %perform apparent density inversion using gravity attraction data
       [pre_density, pre_rho_coeffs] = iterative_density_mapping_attraction(gravity_attraction, up_boundary, low_boundary, height, initial_density, max_iter, tol, alpha,Lon,Lat,nmax);
        ...
        updata_grav_attraction = compute_gravity_anomaly(pre_density, up_boundary, low_boundary, height, nmax, nlat, nlon, R, G, Lat, Lon);%gravity forward modeling to facilitate subsequent differencing
        gravity_attraction(:,3)=prev_gravity(:,3)-updata_grav_attraction(:,3);%obtain the gravity attraction residuals
