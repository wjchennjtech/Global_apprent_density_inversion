function [final_density, final_rho_coeffs] = iterative_density_mapping_attraction(gravity_attraction,up_boundary,low_boundary,height,initial_density,max_iter,tol,alpha,Lon,Lat,nmax)
    nmax1=nmax;    
    nlon=size(Lon,2);nlat=size(Lat,2);R=6378136.3;G=6.673e-11; 
    data2(:,3) = up_boundary(:,3) * 1e3;  
    data1(:,3) = low_boundary(:,3) * 1e3; 
    datat2 = mean(data2(:,3));
    datat1 = mean(data1(:,3));
  ...
  for iter = 1:max_iter
   ...
   % Step 1: Solve for the spherical harmonic coefficients of the first term (the fixed term).
    gravity_attraction(:,3) = gravity_attraction(:,3) / 1e5;  
    [Anm,Bnm] = AB_matrix(gravity_attraction,nlat,nlon);
    [hcnm,hsnm] = LS_SHA(Anm,Bnm,gravity_attraction,nlat,nmax);
    CNM = zeros(nmax+1, size(hcnm,2));
    SNM = zeros(nmax+1, size(hsnm,2));
    for n = 0:nmax
        factor = ((2*n+1)*(n+3))*(1/(R+height)) / ( (((R+datat2)/(R+height))^(n+3) - ((R+datat1)/(R+height))^(n+3) ) * 4*pi*G*(n+1) );
        CNM(n+1,:) = factor * hcnm(n+1,:);
        SNM(n+1,:) = factor * hsnm(n+1,:);
    end
...
% From the second iteration onward, the spherical harmonic coefficients are a combination of (1 - alpha) times the previous coefficients and alpha times the current ones.
     ...
     [Iteration_of_density]=synthesis(lastCnm,lastSnm,nmax1,Lat,Lon);
     ...
     for i = 1:nlat
        for j = 1:nlon
            ...
            fresult((i-1)*nlon+j,3) = Iteration_of_density(i,j);
        end
     end
    ...
    for n = 0:nmax
        for m = 0:n
          ...
          rho_coeffs_final(row,4) = lastSnm(n+1, m+1);
          row = row + 1;
        end
    end
    ...
    if density_rms_change < tol
        fprintf("Density RMS change %.6e is less than the tolerance %.6e, iteration terminated.\n", density_rms_change, tol);
        break;
    end
  
    prev_density(:,3) = fresult(:,3);
    rho(:,3) = fresult(:,3);
  end
